...
if (!plants.length) {
  window.addEventListener("DOMContentLoaded", function () {
    ...
    const message = "No plant records loaded. Most likely data/processed/plants_public_v3.js was not uploaded at that exact path, or GitHub Pages is still serving an older build.";
    if (stats) stats.innerHTML = '<div class="stat"><strong>0</strong><span>No plant data loaded</span></div>';
    if (resultCount) resultCount.textContent = "0 plants shown";
    if (panel) panel.innerHTML = '<p class="empty-detail"><strong>Data load problem.</strong> ' + message + '</p>';
  });
}
