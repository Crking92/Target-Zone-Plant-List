...
(function () {
  ...
  function buildCsv() {
    ...
    const sortedPlants = [...plants].sort((a, b) => {
      const sciA = getValue(a, ["Scientific Name", "Plant_ID"]);
      const sciB = getValue(b, ["Scientific Name", "Plant_ID"]);
      const commonA = getValue(a, ["Common Name(s)", "Common Name", "Common Names"]);
      const commonB = getValue(b, ["Common Name(s)", "Common Name", "Common Names"]);
      return sciA.localeCompare(sciB, undefined, { sensitivity: "base" }) ||
             commonA.localeCompare(commonB, undefined, { sensitivity: "base" });
    });

    const rows = sortedPlants.map(plant => {
